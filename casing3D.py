...
class Casing3D(QtWidgets.QWidget):
    ...
    def on_color_click(self, row, col):
        if col == 1:
            ...
            if color.isValid():
                self.sender().item(row, 1).setBackground(QBrush(QColor(color)))
    ...
    def on_files_change(self, fname):
        if hasattr(self, 'glWidget'):
            ...
            depLength = float(ceil(abs(start - stop))) - 1
            ...
            self.ui.depth.setRange(float(start), float(stop))
            self.ui.depth.setValue(float(stop))
            self.ui.depSelect.setRange(int(start), int(stop))
            self.ui.depSelect.setValue(int(stop))
...
class GLWidget(QtOpenGL.QGLWidget):
    ...
    def update_depth(self, dep):
        if hasattr(self, 'data_len')==False:
            return
        start, stop, step = self.depth
    ...
    def wheelEvent(self, event):
        idx = (int)(self.data_idx - abs(event.angleDelta().y()) / event.angleDelta().y() * 30)
    ...
    def build_xyz_axis(self):
        ...
        glColor3d(1, 0, 0);
        glVertex3d(0, 0, 0);
        glVertex3d(10, 0, 0)  # red a
        glColor3d(0, 1, 0);
        glVertex3d(0, 0, 0);
        glVertex3d(0, 10, 0)  # cylin l
        glColor3d(0, 0, 1);
        glVertex3d(0, 0, 0);
        glVertex3d(0, 0, 10)  # blue b
        glEnd()
        glEndList()
    ...
    def build_data_tool(self):
        ...
        z = DISP_LEN / 2.0
        for i in self.tools:
            id_data, data, angle, line_start, line_stop = (DATA_MFC, DATA_MTD)[i], self.data[i], self.angle[i], (-2, 1)[
                i], (2, 4)[i]
