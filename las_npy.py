...
def msgbox(win, title, msg):
    QtWidgets.QMessageBox.critical(win, utf8(title), utf8(msg))


def import_las(win, ui_files, is_interpre=False):
    fname = QtWidgets.QFileDialog.getOpenFileName(win, utf8('打开LAS文件'), '', '*.las')
    if fname:
        fname = las2npy_nointerpre(fname) if is_interpre else las2npz(fname)
...
def find_tool_range(data, curves):
    ...
    print("idxOD=%d, idxTHK=%d, idxRB=%d, idxDEV=%d, idxODmax=%d, idxTHKmax=%d" % (
    idxOD, idxTHK, idxRB, idxDEV, idxODmax, idxTHKmax))
